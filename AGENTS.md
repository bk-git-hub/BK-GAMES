...
공유 파일을 수정하면 작업 완료 후 공개해도 되는 요약을 `ENGINEERING_LOG.md`에 정리한다.

다른 담당 영역에서 변경이 필요해지면 즉시 멈추고 사용자에게 범위 변경을 보고한다.

## 8. Dev Server 수동 운영 규칙

개발 서버 start/stop/restart는 사용자가 수동으로 수행한다.

에이전트는 사용자의 명시 지시 없이 dev server를 start/stop/restart하지 않는다. 포트 kill 또는 PID 종료도 하지 않는다.
...
서버 재시작이 검증에 필요하면 사용자에게 수동 재시작을 요청한다.
런타임 로그, 캐시, 관측 파일은 커밋하지 않는다.
```
