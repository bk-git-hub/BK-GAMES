...
- Harden remaining blackjack rule options around dealer peek/no-hole-card timing and split option enforcement, including double-after-split, resplit limits, split aces, and hit-split-aces behavior.
...
```text
이번 작업 범위:
- 목표: blackjack insurance and even-money decision foundation
...
- 제외: frontend UI, admin UI, frontend socket client
- 검증: game-server insurance/even-money state transition tests, DB smoke for insurance/even-money wallet idempotency if wallet mutation is added, 전체 workspace 검증
```
