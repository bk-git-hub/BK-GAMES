...
- Horse racing betting was expanded from WIN-only to WIN, QUINELLA, and EXACTA. `racing_bet_selections` now stores ordered or order-insensitive selections, users can buy multiple tickets per race as long as each ticket has a unique `commandId`, and the main racing table now closes betting 30 seconds before start. Verification included migration, main seed, racing wallet smoke, and full workspace typecheck.
- Horse racing realtime server foundation was added. `packages/shared` now exposes `/racing` socket contracts, `apps/game-server` has a `RacingModule`, `RacingGateway`, `GET /racing/tables`, DB-backed racing table config, and in-memory viewer state for `TABLE_JOIN`. Live smoke verified `GET /racing/tables` and a `/racing` socket join returning `TABLE_STATE` plus `TABLE_JOINED`. Next racing backend step should add the fixed 4-minute scheduler and DB race creation before accepting bets.
- Horse racing timing was corrected to a fixed 240-second race interval: 60 seconds for race/result display, 150 seconds of betting, and a 30-second no-bet lock before race start. The DB constraint now requires `betting_timeout_seconds + betting_close_before_start_seconds < race_interval_seconds`, and `GET /racing/tables` exposes `timing.raceAndResultSeconds`. Next racing scheduler work should use fixed 4-minute start slots.
- Horse racing fixed-slot scheduler foundation was added. `ensureScheduledRacingRace` creates or reuses the next future 4-minute race slot, assigns entries from active horses, moves pre-start races through `WAITING -> BETTING -> LOCKING_BETS`, and keeps betting closed before `bettingOpensAt` or after `bettingClosesAt`. `GET /racing/tables` and `/racing` socket state now expose the current `race` snapshot plus `timers.scheduledStartAt`/`bettingClosesAt`. Verification included scheduler smoke, racing wallet smoke, full workspace typecheck, live `GET /racing/tables`, and a `/racing` socket join smoke. Next racing backend step should add bet placement socket events against the scheduled race or start the race runner that advances `LOCKING_BETS` races into `RUNNING`.

### 2026-06-14

- Horse racing socket betting was added. `RACING_CLIENT_EVENTS.BET_PLACE` now accepts `commandId`, `tableId`, `raceId`, `betType`, point-string `amount`, and `raceEntryIds`; the game-server validates the current scheduled race, calls DB-backed `placeRacingBet`, emits public `BET_PLACED` table events, and sends private `wallet:updated` with reason `BET_PLACED`. Impact: frontend can submit WIN, QUINELLA, and EXACTA bets during `state.phase === "BETTING"` using the current `state.race.raceId` and entry IDs from `state.race.entries`; it should handle racing bet errors from the shared socket error code union. Verification included game-server/shared typecheck, full workspace typecheck, racing wallet smoke, and a live `/racing` socket smoke that placed a 100pt WIN bet and received both `BET_PLACED` and private wallet update. Next racing backend step should implement the race runner that moves locked races into `RUNNING`, emits ticks/results, and settles racing bets.
- Horse racing race runner was added. `advanceRacingRaceLifecycle` moves due races from pre-start phases into `RUNNING`, deterministically settles running races after the configured race-duration portion, marks settled races as `ROUND_END` after result display, and pays winning bets through the existing wallet ledger settlement path. `GET /racing/tables` and `/racing` socket state now keep active `RUNNING`/`SETTLED` races visible before switching to the next scheduled race, and race snapshots include start/finish/settlement timestamps, result order, final ranks, and finish times. Impact: frontend should render `RACE_STARTED`, `RACE_TICK`, and `RACE_SETTLED` table events, animate from `event.tick.positions`, and update balances from private racing `wallet:updated` payout events. Verification included `smoke:racing-runner`, `smoke:racing-wallet`, full workspace typecheck, and live `main` observation of `RUNNING -> SETTLED(resultOrder length 6) -> next BETTING` without forcing table state.
- Game-server racing cycle smoke was added. `pnpm --filter game-server smoke:racing-cycle` now creates a temporary user/wallet, waits for `main` to enter `BETTING`, places a 100pt WIN bet over `/racing` Socket.IO, observes `BET_PLACED`, private `wallet:updated`, `RACE_STARTED`, and `RACE_SETTLED`/settlement completion, then deletes only the temporary user data. `socket.io-client` was added as a game-server dev dependency and `pnpm-lock.yaml` was updated. Verification included the new smoke command and full workspace typecheck.
- Horse racing simulation was corrected to produce results from deterministic per-tick race movement instead of pre-aligning ticks to a fixed final order. `packages/shared/src/racing-simulation.ts` now provides the DB runner with a seed-based tick simulation where horses accelerate, slow, burst, and finish at different milliseconds; `racing-runner` locks a deterministic seed at race start and settles from the simulated final tick. Game-server `RACE_TICK` now uses the same movement model locally for public progress/rank snapshots, allowing visible overtakes while keeping settlement reproducible. `@bk-games/db` now depends on internal `@bk-games/shared`; `pnpm-lock.yaml` changed. Verification included racing gateway spec, shared/db/game-server typecheck, full workspace typecheck, and `pnpm --filter @bk-games/db smoke:racing-runner`.
- Horse racing finish timing was tuned. The main race run window is now 43 seconds inside the existing 60-second race/result phase by setting `roundEndDelaySeconds` to 17, and the tick simulation variance was narrowed so finishes cluster more tightly. A 3,000-seed local sample measured average first finish at about 38.82s, average horse finish at about 41.22s, average last finish at about 42.81s, and average finish spread at about 3.99s. Local `main` seed was reapplied with `roundEndDelaySeconds: 17`. Verification included full workspace typecheck and `pnpm --filter @bk-games/db smoke:racing-runner`.
- Shared package dev runtime export coverage was updated for racing simulation. `packages/shared/src/racing-simulation.js` now mirrors the existing source shim pattern used by `socket-events.js` and `types.js`, so `apps/game-server` can resolve `@bk-games/shared` in Nest watch mode after `index.ts` exports `./racing-simulation.js`. Impact: backend and frontend threads can continue importing shared racing simulation/contracts without requiring a prebuild during local development.
- Main horse racing timing was temporarily shortened for frontend development. The active `main` seed now uses a 90-second race interval, 30-second betting timer, 0-second pre-start betting gap, and the existing 60-second race/result window; `racing_tables_betting_close_before_start_check` now allows a zero-second close gap. `packages/db/src/racing-seed.ts` keeps the original standard timing (`150/240/30`) in `standardMainRacingTiming`, and the DB `rules.developmentTiming.restoreTo` value records the same restoration target. Local DB was migrated and `seed:racing-main` was applied; game-server was restarted and `/racing/tables` confirmed `30/90/0`.
- Horse racing betting types were expanded from 3 to 7. Shared/backend now support `WIN`(단승), `PLACE`(연승), `QUINELLA`(복승), `EXACTA`(쌍승), `QUINELLA_PLACE`(복연승), `TRIO`(삼복승), and `TRIFECTA`(삼쌍승). The existing `BET_PLACE` payload shape is unchanged; frontend should vary `raceEntryIds.length` by bet type: 1 for `WIN`/`PLACE`, 2 for `QUINELLA`/`EXACTA`/`QUINELLA_PLACE`, and 3 for `TRIO`/`TRIFECTA`. Local `bkpublic99@gmail.com` was also credited 1000 points through an idempotent `ADMIN_ADJUST` ledger for development testing.
- Main horse racing timing was restored to the standard 4-minute cycle for betting UI implementation. The active `main` seed now uses `150/240/30` again: 150 seconds of betting, 30 seconds of pre-start lock, and 60 seconds for race/result display. `developmentTiming.enabled` is now false, while `developmentTiming.shortcutTiming` keeps the previous `30/90/0` preset available for future fast-cycle testing.
